<#
**This MUST be run by using `. my.ps1` to work.**
It activates the python virtual environment and sets the PowerShell variables $MAESTRO, $MYMC and $MYPY so that they will work from anywhere, including
...
$MAESTRO = Join-Path $thisDir "bin\python\maestro.py" -Resolve # the maestro script, which is not self-executing.
